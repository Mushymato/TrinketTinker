...
#### Added
- ProcSound now has a Chance field ([PR#9 Lintharr](https://github.com/Mushymato/TrinketTinker/pull/9))
- Updated ru and es ([PR#8 ellatuk](https://github.com/Mushymato/TrinketTinker/pull/8))
- Added BugKiller to damage args which kills the bug

#### Fixed
- Height field on projectile was not being checked for shadow logic.

### 1.9.2

#### Fixed
- Inventory `DropContentsAsDebrisCondition` should default to `FALSE`
- Unify RequiredTags logic to use list OR space sep AND
...
- New GSQs
    ...
    - `mushymato.TrinketTinker_TRINKET_HAS_COUNT_ITEMS` check if the trinket's inventory has this many items
