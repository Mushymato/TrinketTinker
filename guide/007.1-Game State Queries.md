...
Check if the trinket item with an inventory has a certain number of the given item.
Only works in context where the Input/Target item is a trinket with tinker data, does not support string trinket Id.

## mushymato.TrinketTinker_TRINKET_HAS_ITEM_CONTEXT_TAG

```
mushymato.TrinketTinker_TRINKET_HAS_ITEM_CONTEXT_TAG <Input|Target> <context_tags+> <item count compare>
```

Check if the trinket item with an inventory has a certain number items matching the given context tag(s).
Context tags are ANY for the top list, but each individual tag can be a space separated list of tags that will be evaluated with AND.
Only works in context where the Input/Target item is a trinket with tinker data, does not support string trinket Id.

## mushymato.TrinketTinker_TRINKET_HAS_COUNT_ITEMS

```
mushymato.TrinketTinker_TRINKET_HAS_COUNT_ITEMS <Input|Target> <item count compare>
```
