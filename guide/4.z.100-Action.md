...
| `Actions` | string | _null_ | Additional actions to run, either this or `Action` must be set for the ability to work. |
| `ActionEnd` | string | _null_ | An action to run when the trinket is unequiped, only used if [ProcOn](4.0-Proc.md) is `Always`. |
| `ActionsEnd` | string | _null_ | Additional actions to run when the trinket is unequiped, only used if [ProcOn](4.0-Proc.md) is `Always`. |
